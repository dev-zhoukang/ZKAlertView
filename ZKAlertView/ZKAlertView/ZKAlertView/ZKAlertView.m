...
@interface ZKAlertViewController : UIViewController
...
- (void)handleShowAlertView:(ZKAlertView *)alertView;
...
@implementation USAlertManager {
    UIWindow *_window;
    ZKAlertView *_showingView; //!< 正在展示的alertView
    NSMutableArray *_alertViews;
    UIView *_maskView;
    ZKAlertViewController *_alertVC;
}
...
+ (instancetype)shareInstance
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        mgr->_alertVC = [ZKAlertViewController new];
...
- (void)handleAlertViewClick
{
    ...
    if ([_alertViews count] > 0) {
        ZKAlertView *alertView = _alertViews.lastObject;
...
- (void)handleShowAlertView:(ZKAlertView *)view
{
...
- (void)showAlertView:(ZKAlertView *)view
{
    ZKAlertViewController *alertVC = (ZKAlertViewController *)_window.rootViewController;
...
@property (nonatomic, copy) void (^forkBtnBlock)();
...
- (void)setup
{
    ...
    NSBundle *bundle = [NSBundle bundleWithPath:[[NSBundle bundleForClass:[ZKAlertView class]] pathForResource:@"ZKAlertView" ofType:@"bundle"]];
    ...
    [_forkBtn addTarget:self action:@selector(forkBtnClick) forControlEvents:UIControlEventTouchUpInside];
...
{
    ZKAlertView *alertView = [self initWithTitle:nil message:message cancelButtonTitle:@"确定" otherButtonTitles:nil];
...
{
    ZKAlertView *alertView = [self initWithTitle:title message:message cancelButtonTitle:@"确定" otherButtonTitles:nil];
    [alertView showWithCompletionBlock:nil];
...
+ (instancetype)initWithTitle:(NSString *)title
        ...
        otherButtonTitleArray:(NSArray *)otherButtonTitleArray;
{
    ZKAlertView *alertView = [ZKAlertView new];
    ...
    if (attributedMsg.length) {
        CGFloat msgTextHeight = [attributedMsg.string stringHeightWithFont:attributedMsg.font?:kMsgFont
                                                                     width:kLabelMaxWidth
                                                               lineSpacing:attributedMsg.lineSpacing];
...
- (void)clickForkBtnBlock:(void(^)())forkBtnBlock
{
    _forkBtnBlock = forkBtnBlock;
}
...
- (void)forkBtnClick
{
    _forkBtnBlock?_forkBtnBlock():nil;
    [self hideAlertView];
}
